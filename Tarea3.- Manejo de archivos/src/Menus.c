...
//ENCONTRAR LA POSICION DE DONDE ESTA LO QUE BUSCO
	cuentas_dat = fopen(rutaCuentas,"rb"); //Para leer su contenido CLIENTES.dat
	if (cuentas_dat == NULL){
		printf("Error al abrir archivo cuentas_dat\n");
	}
		...
		Cuenta temp;
		int i = 0;
		printf("\nTodos los datos VALIDOS almacenados en el archivo cuentas_dat.");
		while( fread(&temp, sizeof(Cuenta), 1, cuentas_dat) != 0 ){
			if( temp.id_cuenta != -5){
				printf("\n****Cuenta %d****", i+1);
				printf("\nEl ID de cuenta es: %d", temp.id_cuenta);
				printf("\nEl ID de cliente es: %d", temp.id_cliente);
				printf("\nEl saldo de la cuenta es: %d", temp.saldo);
				printf("\nSu fecha de Apertura de la cuenta es: %d / %d / %d \n", temp.apertura.dia, temp.apertura.mes, temp.apertura.ano);
			}
			i++;//Leer la siguiente
		}
		fclose(cuentas_dat);
...
//ENCONTRAR LA POSICION DE DONDE ESTA LO QUE BUSCO
				...
				if (cuentas_dat == NULL){
					...
					fclose(cuentas_dat);
					if (aqui != -1){
						temp.id_cuenta = borradoLogico;	 //Osea, -5

						// MODIFICAR LO QUE ENCONTRE EN LA POSICION  A FSEEK
						cuentas_dat = fopen("prueba.dat", "r+b"); //ESCRIBIR EL BORRADO LÓGICO EN CLIENTES.dat
						if (cuentas_dat == NULL) {
							printf("Error al abrir cuentas_dat\n");
						} else {
							printf("\nSe ha actualizado el archivo cuentas_dat.");
							i = 0;
							fseek(cuentas_dat, aqui * sizeof(Cuenta), SEEK_SET);
							fwrite(&temp, sizeof(Cuenta), 1, cuentas_dat);
							fclose(cuentas_dat);
						}
					}
				}
...
void eliminarCliente (Cliente clientes[MAX], Cliente *arrApuntadores[]){
	...
	for(int i=0; i < MAX; i++){
		if ((apunArr + i)->id == idBorrar)
		{
			...
			//Actualizar el archivo Cuentas con lo mas nuevo
			clientes_dat = fopen(rutaCuentas, "wb");//Borrar lo que habia antes en clientes_dat
			fclose(clientes_dat);

			clientes_dat = fopen(rutaCuentas, "a+b");//Borrar lo que habia antes en clientes_dat
			for(i=0; i<posiCuentas; i++)
				fwrite(&clientes[i], sizeof( Cuenta ), 1, clientes_dat);
			fclose(clientes_dat);
			bandera = 1;
//
...
//ENCONTRAR LA POSICION DE DONDE ESTA LO QUE BUSCO
	clientes_dat = fopen(rutaClientes,"rb"); //Para leer su contenido CLIENTES.dat
	if (clientes_dat == NULL){
		printf("Error al abrir archivo clientes_dat\n");
	}
	else{//Se abrió el archivo para leer y escribir
		Cliente temp;
		int i = 0;
    	printf("\nTodos los datos VALIDOS almacenados en el archivo clientes_dat.");
		while( fread(&temp, sizeof(Cliente), 1, clientes_dat) != 0 ){
			if( temp.id != -5){
				printf("\n****Cliente %d****", i+1);
				printf("\nEl ID es: %d", temp.id);
				printf("\nEl nombre del cliente es: %s", temp.nombre);
				printf("\nEl apellido Paterno del cliente es: %s", temp.paterno);
				printf("\nEl apellido Materno del cliente es: %s", temp.materno);
				printf("\nSu fecha de Nacimiento es: %d / %d / %d \n", temp.nacimiento.dia, temp.nacimiento.mes, temp.nacimiento.ano);
			}
			i++;//Leer la siguiente
		}
		fclose(clientes_dat);
